...
export function ExitIntentPopup() {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      window.removeEventListener("open-popup", handleOpenPopup);
    };
  }, [hasShown]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    toast({
      title: "Assessment Requested",
      description:
        "One of our senior consultants will contact you shortly.",
    });
  };
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent
        className="sm:max-w-[500px] border-t-4 border-t-[#FE0105]"
        aria-describedby="exit-desc"
      >
        <DialogHeader>
          <DialogTitle className="text-2xl font-serif text-black dark:text-white">
            Leaving so soon?
          </DialogTitle>
          ...
          <DialogDescription className="text-base mt-2">
            ...
            <span className="font-semibold text-[#0B1F3A] dark:text-white">
              Free Canada PR Assessment
            </span>
            . Discover your CRS score and eligibility today.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Input required placeholder="+971 5X XXX XXXX" />
            </div>
          </div>
          ...
          <Button
            type="submit"
            className="w-full bg-[#FE0105] hover:bg-[#d81418] text-white font-semibold text-lg py-6"
          >
            Get My Free Assessment
          </Button>
        </form>
